 <?php
  ...
  if(isset($_GET['b3_id']))
    {
      $id = $_GET['b3_id'];
      extract($b3->getEditB3($id));  
    }else{
      $id = $_POST['b1_id'];
      extract($b3->getEditB3B1($id));
    }

  if(!empty($_POST['b3_id'])){
    $b1_id          = $_POST['b1_id'];
    $b3_id          = $_POST['b3_id'];
    ...
    if($b3->updateB3($b3_id,$b1_id,$jenis_rawat,$id_diagnosa,$jml_l,$jml_p,$jml_0,$jml_5,$update_by)){
      $sg   = "ok";
...
<form id="forms" method="post" onSubmit="return submitForm('<?php echo $_SERVER['PHP_SELF']; ?>')" class="form-horizontal">
  <fieldset>
    ...
    <div class="modal-content">
        <input type="hidden" id="b1_id" name="b3_id" value="<?php echo $b3_id; ?>" readonly="readonly " />
        ...
        <label class="control-label">Jenis Rawat</label>
          <div class="controls">
            <label>
              <input name="jenis_rawat" type="radio" value="RJ" <?php echo ($jenis_rawat=='RJ')?'checked':''; ?> />
              <span class="lbl"> Rawat Jalan </span>
              <input name="jenis_rawat" type="radio" value="RI" <?php echo ($jenis_rawat=='RI')?'checked':''; ?> />
        ...
        <div class="control-group">
          ...
          <select name="id_diagnosa" id="id_diagnosa" required class="chzn-select" data-placeholder="Pilih Diagnosa.............">
            <option value="<?php echo $id_diagnosa; ?>"><?php echo $diagnosa; ?></option>            
            <?php            
    ...
    <div class="modal-footer">
      ...
      <?php echo "
        <a href='javascript:void(0)' onclick='modal($b1_id)' class='btn btn-primary' ><i class='icon-close icon-white'></i>Tutup</a> "; ?>
    </div>
  </fieldset>
</form>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $(".chzn-select").chosen();
    $('#spinner1').ace_spinner({value:<?php echo $jml_l; ?>,min:0,max:200,step:1, btn_up_class:'btn-info' , btn_down_class:'btn-info'})
    .closest('.ace-spinner')
    .on('changed.fu.spinbox', function(){
      //alert($('#spinner1').val())
    });
    $('#spinner2').ace_spinner({value:<?php echo $jml_p; ?>,min:0,max:200,step:1, btn_up_class:'btn-info' , btn_down_class:'btn-info'})
    .closest('.ace-spinner')
    .on('changed.fu.spinbox', function(){
      //alert($('#spinner1').val())
    });
    $('#spinner3').ace_spinner({value:<?php echo $jml_0; ?>,min:0,max:200,step:1, btn_up_class:'btn-info' , btn_down_class:'btn-info'})
    .closest('.ace-spinner')
    .on('changed.fu.spinbox', function(){
      //alert($('#spinner1').val())
    });
    $('#spinner4').ace_spinner({value:<?php echo $jml_5; ?>,min:0,max:200,step:1, btn_up_class:'btn-info' , btn_down_class:'btn-info'})
    .closest('.ace-spinner')
    .on('changed.fu.spinbox', function(){
      //alert($('#spinner1').val())
    });
